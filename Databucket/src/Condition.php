...
class Condition
{
    ...
    /**
     * If empty return same value else return where condition
     *
      * @param mixed $value
     * @param mixed $comparable
     *
     * @return array
     */
    public static function whereReturnIfNull($value, $comparable)
    {
        if((is_array($comparable) && array_key_exists($value, $comparable))){
            return (is_array($comparable) && array_key_exists($value, $comparable));
        }else{
            return $value;
        }
